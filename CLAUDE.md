...
- `cd src/deployment/seed && npm install && npm run seed` — seeds the `books` (93,624 docs) and `genres` (1 doc) collections from a vendored dataset (`data/seed-data.tar.gz`). Requires `BOOKSTORE_SEED_DB_CONNECTION_STRING` in `src/deployment/seed/.env` (loaded via `dotenv`).
