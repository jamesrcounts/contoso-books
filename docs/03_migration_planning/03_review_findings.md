...
A report is only useful if you can read it. In this task you interpret the assessment output, locate the **Critical** finding that blocks migration, and understand *why* it is unsupported — which is what tells you how to fix it in Task 04.
...
Under the **Features** category you will find a **Critical** finding for an unsupported aggregation operator: **`$function`**. The finding includes the operator name, an actionable recommendation, and a **usage-frequency** value — the count comes from the reading-insights report you exercised in Exercise 01, Task 06.
